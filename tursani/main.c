...
void loadsprites(void)
{
    ...
    //load bonus shot
    bonus_shot_image = load_bitmap("powerup.bmp", NULL);
    ...
    temp = load_bitmap("player1.bmp", NULL);
    ...
    //load bullet images
    bullet_images[0] = load_bitmap("bullets2.bmp", NULL);
    ...
    temp = load_bitmap("enemy1.bmp", NULL);
    for (n=0; n<3; n++){
        enemy_plane_images[n] = grabframe(temp,32,32,0,0,3,n);
	}

    temp = load_bitmap("enemy2.bmp", NULL);
    for (n=0; n<3; n++){
        enemy_plane2_images[n] = grabframe(temp,32,32,0,0,3,n);
	}

    temp = load_bitmap("enemy3.bmp", NULL);
    for (n=0; n<3; n++){
        enemy_plane3_images[n] = grabframe(temp,32,32,0,0,3,n);
	}
    destroy_bitmap(temp);
    ...
    for (n=0; n<MAX_ENEMIES; n++)
    {
        ...
        enemy_planes[n]->xspeed = (rand() % 5 - 3);
		...
		enemy_planes2[n] = (SPRITE*)malloc(sizeof(SPRITE));
        enemy_planes2[n]->alive = 0;
        enemy_planes2[n]->x = rand() % 100 + 20;
        enemy_planes2[n]->y = 0;
        enemy_planes2[n]->width = enemy_plane_images[0]->w;
        enemy_planes2[n]->height = enemy_plane_images[0]->h;
        enemy_planes2[n]->xdelay = 4;
        enemy_planes2[n]->ydelay = 4;
        enemy_planes2[n]->xcount = 0;
        enemy_planes2[n]->ycount = 0;
        enemy_planes2[n]->xspeed = (rand() % 8 - 3);
        enemy_planes2[n]->yspeed = 1;
        enemy_planes2[n]->curframe = 0;
        enemy_planes2[n]->maxframe = 2;
        enemy_planes2[n]->framecount = 0;
        enemy_planes2[n]->framedelay = 10;
        enemy_planes2[n]->animdir = 1;

		enemy_planes3[n] = (SPRITE*)malloc(sizeof(SPRITE));
        enemy_planes3[n]->alive = 0;
        enemy_planes3[n]->x = rand() % 100 + 50;
        enemy_planes3[n]->y = 0;
        enemy_planes3[n]->width = enemy_plane_images[0]->w;
        enemy_planes3[n]->height = enemy_plane_images[0]->h;
        enemy_planes3[n]->xdelay = 4;
        enemy_planes3[n]->ydelay = 4;
        enemy_planes3[n]->xcount = 0;
        enemy_planes3[n]->ycount = 0;
        enemy_planes3[n]->xspeed = (rand() % 10 - 3);
        enemy_planes3[n]->yspeed = 1;
        enemy_planes3[n]->curframe = 0;
        enemy_planes3[n]->maxframe = 2;
        enemy_planes3[n]->framecount = 0;
        enemy_planes3[n]->framedelay = 10;
        enemy_planes3[n]->animdir = 1;
    }
...
{
    int n;
	play_sample(explodes, volume, panning, pitch, FALSE);
    for (n=0; n<MAX_EXPLOSIONS; n++)
    {
...
void updatebonuses()
{
    ...
    if (bonus_shot->alive)
    {
        ...
        if (inside(x,y,x1,y1,x2,y2))
        {
			...
			play_sample(powerUp, volume, panning, pitch, FALSE);
            bonus_shot->alive=0;
        }
    }
...
void updatebullet(SPRITE *spr)
{
    ...
    for (n=0; n<MAX_ENEMIES; n++)
    {
		...
		if (enemy_planes2[n]->alive)
        {
            ...
            //get enemy plane bounding rectangle
            x1 = enemy_planes2[n]->x;
            y1 = enemy_planes2[n]->y - yoffset;
            x2 = x1 + enemy_planes2[n]->width;
            y2 = y1 + enemy_planes2[n]->height;
		...
		if (enemy_planes3[n]->alive)
        {
            ...
            //get enemy plane bounding rectangle
            x1 = enemy_planes3[n]->x;
            y1 = enemy_planes3[n]->y - yoffset;
            x2 = x1 + enemy_planes3[n]->width;
            y2 = y1 + enemy_planes3[n]->height;
...
void fireatenemy()
{
    ...
    for (n=0; n<MAX_BULLETS; n++)
    {
        if (!bullets[n]->alive)
        {
			...
			play_sample(shooting, volume, panning, pitch, FALSE);
            return;
        }
    }
}
...
void updateenemyplanes2()
{
    ...
    for (n=0; n<MAX_ENEMIES; n++)
    {
        ...
        {
            c++;
            updatesprite(enemy_planes2[n]);
            bouncex_warpy(enemy_planes2[n]);

            //is plane visible on screen?
            if (enemy_planes2[n]->y > yoffset-32 && enemy_planes2[n]->y < yoffset + HEIGHT+32)
            {
                //draw enemy plane
                draw_sprite(buffer, enemy_plane2_images[enemy_planes2[n]->curframe],
                    enemy_planes2[n]->x, enemy_planes2[n]->y - yoffset);
            }
        }
        //reset plane
        else
        {
            enemy_planes2[n]->alive++;
            enemy_planes2[n]->x = rand() % 100 + 50;
            enemy_planes2[n]->y = yoffset - 2000 + rand() % 2000;
        }
    }
}

void updateenemyplanes3()
{
    int n, c=0;
    ...
    for (n=0; n<MAX_ENEMIES; n++)
    {
        ...
        {
            c++;
            updatesprite(enemy_planes3[n]);
            bouncex_warpy(enemy_planes3[n]);

            //is plane visible on screen?
            if (enemy_planes3[n]->y > yoffset-32 && enemy_planes3[n]->y < yoffset + HEIGHT+32)
            {
                //draw enemy plane
                draw_sprite(buffer, enemy_plane3_images[enemy_planes3[n]->curframe],
                    enemy_planes3[n]->x, enemy_planes3[n]->y - yoffset);
            }
        }
        //reset plane
        else
        {
            enemy_planes3[n]->alive++;
            enemy_planes3[n]->x = rand() % 100 + 50;
            enemy_planes3[n]->y = yoffset - 2000 + rand() % 2000;
        }
    }
}
...
void initialize()
{
	...
	shooting = load_sample("shooting.wav");
    if (!shooting) 
    {
	...
	explodes = load_sample("explode.wav");
    if (!explodes) 
    {
        allegro_message("Error reading wave file");
	...
	splashscreen = load_bitmap("splashscreen.bmp", NULL);
	gameoverscreen = load_bitmap("gameover.bmp", NULL);
	nextlevelscreen = load_bitmap("nextlevel.bmp", NULL);
	clear(buffer);

    //load the Mappy file
	if (MapLoad("spacelevel1.fmp") != 0)
    {
...
int main (void)
{
    ...
    initialize();
    loadsprites();
	blit (splashscreen, screen, 0, 0, 0, 0, SCREEN_W-1, SCREEN_H-1);
	while(!key[KEY_ENTER]){
    ...
    while (!gameover)
	{
        checkinput();
		if(level ==2 && newLevel){
			blit (nextlevelscreen, screen, 0, 0, 0, 0, SCREEN_W-1, SCREEN_H-1);
			while(!key[KEY_ENTER]){
			
			}
			for(i = 0; i < MAX_ENEMIES; i++){
				enemy_planes[i]->alive = 0;
			}
			MapLoad("spacelevel2.fmp");
			newLevel = 0;
		}
		...
		if(level == 3 && newLevel){
			...
			for(i = 0; i < MAX_ENEMIES; i++){
				enemy_planes[i]->alive = 0;
				enemy_planes2[i]->alive = 0;
			}
			MapLoad("spacelevel3.fmp");
			newLevel = 0;
		}
        updatescroller();
	...
	blit (gameoverscreen, screen, 0, 0, 0, 0, SCREEN_W-1, SCREEN_H-1);
	textprintf_ex(screen,font,320,240,WHITE,-1,
        "SCORE %d", score);
	while(!key[KEY_ESC]){
	...
	destroy_bitmap(splashscreen);
	destroy_bitmap(gameoverscreen);
	destroy_bitmap(nextlevelscreen);
    ...
    for (n=0; n<3; n++)
    {
		...
		destroy_bitmap(enemy_plane2_images[n]);
		destroy_bitmap(enemy_plane3_images[n]);
    }
    ...
    for (n=0; n<MAX_ENEMIES; n++){
        free(enemy_planes[n]);
		free(enemy_planes2[n]);
		free(enemy_planes3[n]);

	}
	stop_midi();
	destroy_midi(music);
	destroy_sample(explodes);
	destroy_sample(powerUp);
	destroy_sample(shooting);
	allegro_exit();
	return 0;
}
END_OF_MAIN()
